...
$uxPageBuilderVersion = "1.0.0";

if (!defined('ABSPATH')) exit;

class UX_PageBuilder{
	
	function UX_PageBuilder() {
		add_action('init', array( &$this, 'init'), 1000);
		add_action('admin_init', array( &$this, 'enqueue'), 1000);
	}
	
	function init() {
		add_action('wp_enqueue_scripts', array( &$this, 'theme_enqueue'), 150);
		...
		if(count($ux_pb_modules) > 0){
			foreach($ux_pb_modules as $id => $modules){
				$file_name = $id . '.php';
				require_once dirname(__FILE__) . '/modules/' . $file_name;
			}
		}
	...
	function enqueue() {
		...
		wp_enqueue_script('ux-pb-googlemap', 'https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false', array('jquery'), '3.0', false);
	...
	function theme_enqueue() {
		...
		if($post){
			...
			if($ux_pb_switch == 'pagebuilder'){
				...
				// if($this->has_module('carousel') || $this->has_module('client') || $this->has_module('portfolio')){
				// 	wp_enqueue_script('ux-interface-caroufredsel', plugins_url("js/jquery.caroufredsel.js", __FILE__), array( 'jquery' ), '6.2.1', true);
				// }
				if($this->has_module('google-map')){
					wp_enqueue_script('ux-interface-googlemap', 'https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false', array('jquery'), '3.0.0');
				}
	
				wp_enqueue_script('ux-interface-main', plugins_url("js/main.js", __FILE__), array( 'jquery' ), '0.0.1', true);
				wp_enqueue_script('ux-pb-theme-script', plugins_url("js/theme.pagebuilder.js", __FILE__), array( 'ux-interface-theme' ), '0.0.1', true);
