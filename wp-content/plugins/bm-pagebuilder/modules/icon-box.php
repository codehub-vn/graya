...
function ux_pb_module_iconbox($itemid){
	...
	if($module_post){
		...
		$hyperlink_before  = $link == 'on' ? '<a href="' . esc_url($hyperlink) . '" target="_blank" class="iconbox-a">' : false;
		$hyperlink_after   = $link == 'on' ? '</a>' : false;
		$title             = $title ? '<h5 class="iconbox-h5">' .esc_html($title). '</h5>' : false;
        ...
        c-1.706,1.657-2.773,4.922-2.369,7.262l7.628,44.251c0.404,2.339-0.992,3.349-3.1,2.248L109.339,305.159z" fill="<?php echo esc_attr($mask_color); ?>"/></g></svg>
            ...
            <div class="icon_text">
            	<?php if($layout == 'icon_mouseover'){
					echo str_replace('<a ', '<a class="iconbox-hide-a" ', balanceTags($hyperlink_before));
					echo '<div class="icontext-inn"><div class="middle-ux">';
...
function ux_pb_module_iconbox_select($fields){
	$fields['module_iconbox_layout'] = array(
		...
		//array('title' => __('Mouseover Content ','ux'), 'value' => 'icon_mouseover')
	);
