	...
	if($module_post){
		...
		switch($type){
			default:
				...
				$fitrow         = $type == 'standard_list' ? ' grid_list' : ' masonry'; 
		...
		<?php break; 
			...
			case 'custom_grid':
				...
				if($portfolio_query){ ?>
                    ...
                    <div class="list-layout row-fluid">
	...
	if($module_post){
                    ...
                    <div class="inside <?php echo esc_attr($animation_style); ?>" <?php echo balanceTags($animation_end); ?> style=" <?php echo esc_attr($inside_style); ?>">
                        ...
                        <div class="portfolio-standatd-tit-wrap text-center">
                            <h2 class="portfolio-standatd-tit">
								...
								<?php  
									if ( $tags == 'on') { ?>
								<div class="portfolio-standatd-tags"><?php the_tags('', '  '); ?></div>
								<?php } ?>
                            </h2>
                        </div>
		...
		foreach($portfolio_query as $num => $post){ setup_postdata($post);
			...
			if($hover_effect == 'flip'){ ?>
                <div class="<?php echo esc_attr(trim($output, $separator)); ?> <?php echo esc_attr($width_item); ?> isotope-item container3d">
                    <div class="inside card <?php echo esc_attr($animation_style); ?>" <?php echo balanceTags($animation_end); ?> style=" <?php echo esc_attr($inside_style); ?>">
                        <div class="flip_wrap_back back face">
                            <div class="flip_wrap_back_con centered-ux" style=" <?php echo esc_attr($back_con_style); ?>">
                                <h2 class="flip_wrap_back_con_h2" style=" <?php echo esc_attr($back_tit_style); ?>"><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" class="<?php echo esc_attr($enable_ajax); ?> flip_wrap_back_con_a"><?php the_title(); ?></a></h2>
                                <?php if($ux_portfolio){ ?>
                                    <ul class="hover_thumb_wrap lightbox-photoswipe">
                                        <?php foreach($ux_portfolio as $num => $image){
                                            ...
                                            if($num < 3){ ?>
                                                <li class="hover_thumb_unit" data-lightbox="true"><a href="<?php echo esc_url($thumb_src_full[0]); ?>" title="<?php echo esc_attr(get_the_title($image)); ?>" class="imgwrap lightbox-item" data-rel="post<?php the_ID(); ?>" data-size="<?php echo $data_size; ?>"><img class="flip-hover-back-thumb-img" width="50" height="50" src="<?php echo esc_url($thumb_src[0]); ?>" /></a></li>
                                            <?php
                                            }
                                        } ?>
                                    </ul>
                            ...
                            <div class="flip_wrap_back_bg <?php echo esc_attr($bg_color); ?>" style=" <?php echo esc_attr($back_bg_style); ?>"></div>
                        </div>
                        <?php if(has_post_thumbnail()) { ?>
							<img src="<?php echo esc_url($thumb_src_preview[0]); ?>" width="<?php echo esc_attr($thumb_src_preview[1]); ?>" height="<?php echo esc_attr($thumb_src_preview[2]); ?>" class="front face">
						<?php } ?>
                    
                    </div><!--End inside-->
                </div>
            <?php }else if($hover_effect == 'mask'){ ?>
            	<div class="<?php echo esc_attr(trim($output, $separator)); ?> <?php echo esc_attr($width_item); ?> isotope-item mask-hover">
                    <div class="inside <?php echo esc_attr($animation_style); ?>" <?php echo balanceTags($animation_end); ?> style=" <?php echo esc_attr($inside_style); ?>">

                    	<div class="mask-hover-inn">
							<img src="<?php echo esc_url($thumb_src_preview[0]); ?>" width="<?php echo esc_attr($thumb_src_preview[1]); ?>" height="<?php echo esc_attr($thumb_src_preview[2]); ?>" class="mask-hover-inn-img isotope-list-thumb" />
							<div class="mask-hover-caption">
								<span class="mask-hover-caption-block centered-ux <?php echo esc_attr($bg_color); ?> lightbox-photoswipe">

									<p data-lightbox="true"><a href="<?php echo esc_url($thumbnail_url[0]); ?>" class="lightbox-item" title="<?php the_title(); ?>" data-size="<?php echo $data_size; ?>"><?php esc_html_e('PREVIEW','ux'); ?><img src="<?php echo esc_url($thumb_src_preview[0]); ?>" class="hidden" /></a></p>
									<p><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" class="<?php echo esc_attr($enable_ajax); ?>"><?php esc_html_e('READMORE','ux'); ?></a></p>
			...
			<?php }else{ ?>
                <div class="<?php echo esc_attr(trim($output, $separator)); ?> <?php echo esc_attr($width_item); ?> isotope-item captionhover">
                    <div class="inside <?php echo esc_attr($animation_style); ?>" <?php echo balanceTags($animation_end); ?> style=" <?php echo esc_attr($inside_style); ?>">
                        <figure style=" <?php echo esc_attr($bg_color); ?>">
                            <div class="img_wrap">
								<?php if(has_post_thumbnail()) { ?>
                            ...
                            <figcaption class="<?php echo esc_attr($bg_color); ?> lightbox-photoswipe">
                                <h2 class="captionhover-h2"><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" class="<?php echo esc_attr($enable_ajax); ?> captionhover-tit-a"><?php the_title(); ?></a></h2>
                                <div class="btn_wrap" data-lightbox="true"><a href="<?php echo esc_url($thumbnail_url[0]); ?>" class="captionhover-icon-a lightbox-item" data-size="<?php echo $data_size; ?>">
									<?php if(has_post_thumbnail()) { ?>
                                        <img src="<?php echo esc_url($thumb_src_preview[0]); ?>" class="hidden">
                                    <?php } ?>
                                    <i class="captionhover-icon-i icon-m-view"></i></a><a href="<?php the_permalink(); ?>" class="more captionhover-icon-a"><i class="captionhover-icon-i icon-m-more"></i></a>
                                </div>
                            </figcaption>
                        </figure>
                    </div><!--End inside-->
		...
		?>
			<?php if(has_post_thumbnail()) { ?>
				<section class="interlock-item <?php echo esc_attr($animation_style); ?> lightbox-photoswipe" <?php echo balanceTags($animation_end); ?>>
					...
					<div class="iterlock-caption">
						<h2><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" class="<?php echo esc_attr($enable_ajax); ?>"><?php the_title(); ?></a></h2>
						<?php if ($post_meta != 'off') { ?>
                            <ul class="blog_meta hidden-phone">
                                <li class="blog_meta_cate"><?php esc_html_e('In ','ux');  the_category('&sbquo; '); ?></li>
                            </ul><!--End .blog_meta-->
                        <?php } ?>
						<div class="iterblock-expt hidden-phone"><?php the_excerpt(); ?></div>
						<a href="<?php the_permalink(); ?>" class="iterblock-more ux-btn" title="<?php the_title(); ?>" ><?php echo balanceTags($learnmore); ?><span class="fa fa-long-arrow-right"></span></a>
					</div>
				</section>
			<?php } ?>
		<?php
