...
<div class="cart_totals <?php if($woocommerce->customer->has_calculated_shipping()) echo 'calculated_shipping'; ?>">
	...
	<?php if(!$woocommerce->shipping->enabled || $available_methods || !$woocommerce->customer->get_shipping_country() || !$woocommerce->customer->has_calculated_shipping()){ ?>
		...
		<h3><?php _e('Shopping bag summary', 'woocommerce'); ?></h3>
		...
		<table cellspacing="0">
			<tbody>
				...
				<?php if($woocommerce->cart->get_discounts_before_tax()){ ?>
					...
					<tr class="discount">
						<th><?php _e('Cart Discount', 'woocommerce'); ?> <a href="<?php echo esc_url(add_query_arg( 'remove_discounts', '1', $woocommerce->cart->get_cart_url())); ?>"><?php _e('[Remove]', 'woocommerce'); ?></a></th>
				...
				<?php // Show the tax row if showing prices exclusive of tax only
				if($woocommerce->cart->tax_display_cart == 'excl'){
				...
				<?php if($woocommerce->cart->get_discounts_after_tax()){ ?>
					...
					<tr class="discount">
						<th><?php _e('Order Discount', 'woocommerce'); ?> <a href="<?php echo esc_url(add_query_arg('remove_discounts', '2', $woocommerce->cart->get_cart_url())); ?>"><?php _e('[Remove]', 'woocommerce'); ?></a></th>
				...
				<tr class="total">
					...
					<td>
						...
						<?php // If prices are tax inclusive, show taxes here
		...
		<?php if($woocommerce->cart->get_cart_tax()){ ?>
			...
			<p><small>
				<?php $estimated_text = ($woocommerce->customer->is_customer_outside_base() && !$woocommerce->customer->has_calculated_shipping()) ? sprintf(' ' . __( ' (taxes estimated for %s)', 'woocommerce'), $woocommerce->countries->estimated_for_prefix() . $woocommerce->countries->countries[$woocommerce->countries->get_base_country()]) : '';
		...
		if(!$woocommerce->customer->get_shipping_state() || !$woocommerce->customer->get_shipping_postcode()){ ?>
